...
class DnnMapper(Node):
    def __init__(self):
        ...
        self.path_directory_yolo_detection_visualizations = os.path.join(output_directory, "visualizations")

        if not os.path.exists(self.path_directory_yolo_detection_visualizations):
            os.makedirs(self.path_directory_yolo_detection_visualizations)
        
        if self.do_run_yolo_detection:
            ...
            if has_detections:
                if self.do_publish_visualizations:
                    ...
                    if self.do_save_visualizations:
                        path_file_results_visualization = os.path.join(self.path_directory_yolo_detection_visualizations, 'visualization_' + self.time_to_nanoseconds_string(stereo_image_message.pose.header.stamp)  + '.jpg' )
...
def main(args=None):
    ...
    dnn_mapper = DnnMapper()
    rclpy.spin(dnn_mapper)
    dnn_mapper.destroy_node()
    rclpy.shutdown()
